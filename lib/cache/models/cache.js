...
Cache.prototype.findOne = async function(key) {
	try {
		...
		if (!keyValue) {
			...
			if (kvpHasExpired.apply(this, keyValue.updatedAt)) {
				logger.info('value has expired');
...
function kvpHasExpired(lastUpdate) {
	...
	logger.info(moment(lastUpdate).utc().toISOString());
	let now = moment().utc();
	logger.info(now.toISOString());
	let timeSinceLastUpdate =  now.diff(moment(lastUpdate), 'ms');
	logger.info(timeSinceLastUpdate);
	return timeSinceLastUpdate > this.ttl;
}
